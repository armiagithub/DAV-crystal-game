...
class ArenaScene:
    def __init__(self, screen: pygame.Surface, player_class: str, username: str, max_levels: int = 50):
        ...
        saved = LocalProgress.load_progress(username)
        if saved:
        ...
        for m in list(self.mobs):
            if not m.is_alive():
                ...
                try:
                    ...
                    pass
        if gained:
            self.player.gain_crystals(gained)
            self.shop_message = f"+{gained} crystals (total {self.player.crystals})"

        if self.current_wave > 0 and all(not m.is_alive() for m in self.mobs):
    ...
    def draw(self):
        ...
        self._draw_text(f"Crystals: {self.player.crystals}", 340, 8)
    ...
    def _draw_shop_overlay(self):
        ...
        title = f"Shop - Level {self.level_no} - Crystals: {self.player.crystals}"
        ...
        for idx, it in enumerate(items, start=1):
            ...
            color = (200, 200, 200) if self.player.crystals >= it.cost else (120, 120, 120)
            self.screen.blit(self.font.render(line, True, color), (80, y))
            y += 28
        help_text = "Press number to buy, ESC to close shop."
        ...
        if ev.type == pygame.MOUSEBUTTONDOWN and ev.button == 1 and not self.show_shop_overlay:
            ...
            if ev.unicode.isdigit() and not self.show_shop_overlay:
                ...
                if self.awaiting_replace and self.pending_purchase_item:
                    digit = ev.unicode
                    slot_idx = (int(digit) - 1) if digit != "0" else 9
                    success, info = self.current_shop.buy(self.player, self.pending_purchase_item.name, replace_index=slot_idx)
                    if success:
                    ...
                    if 0 <= idx < len(items):
                        item = items[idx]
                        result, info = self.current_shop.buy(self.player, item.name)
                        if result:
                            ...
                            if info == "inventory_full":
                                ...
                                self.shop_message = "Not enough crystals."
                            else:
